    ...
    for departement in perimetres["departements"]:
        ...
        enveloppes["departements"]["dsil"].append(
            DsilEnveloppeFactory(
                ...
                deleguee_by=Enveloppe.dsil_objects.get(
                    perimetre__region=departement.region
                ),
            )
        )
    for arrondissement in perimetres["arrondissements"]:
        enveloppes["arrondissements"]["detr"].append(
            DetrEnveloppeFactory(
                ...
                deleguee_by=Enveloppe.detr_objects.get(
...
def test_get_enveloppes_from_region_perimetre(perimetres, enveloppes_from_perimetre):
    ...
    assert enveloppes.filter(dotation__label="DETR").count() == 0
    assert enveloppes.filter(dotation__label="DSIL").count() == 3
...
def test_get_enveloppes_from_departement_perimetre(
    ...
    assert enveloppes.filter(dotation__label="DETR").count() == 2
    assert enveloppes.filter(dotation__label="DSIL").count() == 2
    assert enveloppes.filter(perimetre__arrondissement__isnull=True).count() == 2
...
def test_get_enveloppes_from_arrondissement_perimetre(
    ...
    assert enveloppes.filter(dotation__label="DETR").count() == 1
    assert enveloppes.filter(dotation__label="DSIL").count() == 1
