...
@pytest.fixture
def enveloppe(perimetre_departement):
...
def test_simulation_view_has_correct_detr_category_choices_for_departemental_user(
    departemental_user, simulation
):
...
def test_project_list_view_has_previous_year_category_choices_if_needed(
    ...
    detr_category_displayed = CategorieDetrFactory(annee=2024, departement=departement)
    wrong_year_category = CategorieDetrFactory(annee=2023, departement=departement)
    wrong_departement_category = CategorieDetrFactory(
        annee=2025, departement=DepartementFactory()
    )
...
def test_filter_simulation_view_on_category_detr(
