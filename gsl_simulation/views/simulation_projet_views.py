...
class ProgrammationStatusUpdateView(OpenHtmxModalMixin, UpdateView):
    ...
    def get_success_message(self):
        if self.new_project_status == PROJET_STATUS_PROCESSING:
            PROCESSING_PROJET_STATUS_TO_MESSAGE = {
            ...
            return PROCESSING_PROJET_STATUS_TO_MESSAGE[self.kwargs["status"]]

        PROJET_STATUS_TO_MESSAGE = {
            PROJET_STATUS_ACCEPTED: f"La demande de financement avec la dotation {self.object.enveloppe.dotation} a bien été acceptée avec un montant de {euro(self.object.montant, 2)}.",
            PROJET_STATUS_REFUSED: f"La demande de financement avec la dotation {self.object.enveloppe.dotation} a bien été refusée.",
            PROJET_STATUS_DISMISSED: f"La demande de financement avec la dotation {self.object.enveloppe.dotation} a bien été classée sans suite.",
        }
        ...
        if self.new_project_status in [PROJET_STATUS_REFUSED, PROJET_STATUS_DISMISSED]:
            ...
            if (
                self.kwargs["status"] == PROJET_STATUS_REFUSED
                and self.object.projet.dotationprojet_set.filter(
                    status=PROJET_STATUS_DISMISSED
                ).exists()
            ):
                other_dotation = (
                    self.object.projet.dotationprojet_set.filter(
                        status=PROJET_STATUS_DISMISSED
                    )
                    .first()
                    .dotation
        ...
        return PROJET_STATUS_TO_MESSAGE[self.new_project_status] + ds_message
