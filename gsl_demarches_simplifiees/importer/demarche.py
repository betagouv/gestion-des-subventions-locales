...
def save_demarche_from_ds(demarche_number):
    ...
    extract_ds_choice_libelles(demarche_data, demarche)
...
def refresh_categories_operation_detr(demarche_number):
    demarche = Demarche.objects.get(ds_number=demarche_number)
    if demarche.raw_ds_data:
        extract_ds_choice_libelles(demarche.raw_ds_data, demarche)
    else:
        save_demarche_from_ds(demarche_number)
...
def extract_ds_choice_libelles(demarche_data, demarche: Demarche):
    """
    try:
    ...
    for ds_field in demarche_data["activeRevision"]["champDescriptors"]:
        if ds_field["__typename"] not in (
            "DropDownListChampDescriptor",
            "MultipleDropDownListChampDescriptor",
        ):
            continue
        try:
            field_mapping = FieldMappingForComputer.objects.get(
                demarche=demarche, ds_field_id=ds_field["id"]
            )
        except FieldMappingForComputer.DoesNotExist:
            continue

        for sort_order, label in enumerate(ds_field["options"], start=1):
            Dossier._meta.get_field(
                field_mapping.django_field
            ).related_model.objects.update_or_create(
                label=label,
                demarche=demarche,
                demarche_revision=demarche_revision,
                defaults={"tri": sort_order},
            )
    """
