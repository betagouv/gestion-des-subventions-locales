...
def test_init_dossier_converter(ds_dossier_data, dossier):
    # tester aussi dossier_converter.ds_id_to_django_field
    # 2 fixtures fieldmapping for computer
    ...
    # mapping_annotation = FieldMappingForComputer.objects.create(
    #    ds_field_id="TEST_ID_un_champ_annotation",
    #    django_field=Dossier._MAPPED_ANNOTATIONS_FIELDS[0].name, demarche=dossier.ds_demarche)
    dossier_converter = DossierConverter(ds_dossier_data, dossier)
    assert len(dossier_converter.ds_fields_by_id) == len(
        ds_dossier_data["champs"]
    )  # + len(ds_dossier_data["annotations"])
